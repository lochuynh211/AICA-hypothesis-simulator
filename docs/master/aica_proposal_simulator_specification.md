...
| Preference | Oshi information | Oshi artists | `oshi_artists` — array of `{artist_id, oshi_type, enthusiasm}` (`oshi_type` enum: `artist`, `artist_member`, `group`, `character`, `voice_actor`, `franchise`, `creator`, `other`; `enthusiasm` — number 0–1) | Match each registered favorite (multiple allowed, each with its own 熱狂度/enthusiasm) to concrete catalog items — exact/member/group/character/related-entity matches, scored by the MAX enthusiasm among matched artists. | P2/P3 | Slides 68–72, 79; normalized UPro identity; feature 025 slice S2 replaces the single `oshi_id`/`oshi_type` pair with a list |
