...
> **P2 realization — Soundcharts-grounded generation (2026-07-16).** Rather than inventing audio numbers with an LLM alone, P2 grounds the catalog in **real data harvested from the Soundcharts API** (the customer has no Spotify access) and maps it into the frozen schema — keeping **real song/artist names and verbatim real audio** for reviewer trust, while IDs/URLs stay synthetic (`synthetic-` / `.invalid`). The approved design is `docs/master/p2-soundcharts-grounded-data-generation-design.md`, which defines the **S0–S9 pipeline** (coverage plan → three LLM narrowing tiers: web-research, search-narrowing, blind test-case judging → deterministic bin/map/validate/freeze → worlds/profiles → P6 contrast certification) and records the freeze amendments this requires (see data spec §0). The integrity firewall is preserved: **catalog selection is coverage-driven, never score-driven**, and the LLM judges expected **test-case labels blind** with the P6 score as a cross-check. P2 also emits a labeled **test-case set** alongside the label-free catalog. Suggested build staging: **P2a** catalog (S0–S6), **P2b** worlds/profiles (S7), **P2c** LLM judge + certification + test cases (S8–S9).
...
- Implement the **offline generator tool** (a repository-defined command producing committed, versioned JSON) with the three staged passes from the data spec:
  - **PASS 1 — Track objects:** allocate stable synthetic Artist/Album/Track IDs (all prefixed `synthetic-`), names/releases *(P2 §0/D1: **real** names + real release data kept from Soundcharts)*, consistent artist references, exact `spotify_track` field names/types, `.invalid` URLs, `is_playable: true` / `is_local: false` for standard records. Free-form prose rejected.
