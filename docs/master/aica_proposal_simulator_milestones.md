...
> **Soundcharts-grounded generation.** Rather than inventing audio numbers with an LLM alone, P2 grounds the catalog in **real data harvested from the Soundcharts API** (the customer has no Spotify access) and maps it into the frozen schema — keeping **real song/artist names and verbatim real audio** for reviewer trust, while IDs/URLs stay synthetic (`synthetic-` / `.invalid`). The approved design is `docs/master/p2-soundcharts-grounded-data-generation-design.md`, which defines the **S0–S9 pipeline** with **two selectable candidate-acquisition strategies** (`candidate_source` flag): `soundcharts_search` (original LLM-planned search/narrowing — unavailable on the current subscription, kept as fallback) and `isrc_resolved` (web-grounded LLM song-naming → MusicBrainz+Deezer ISRC resolution → `by-isrc` fetch), both converging on the same deterministic bin/map/validate/freeze → worlds/profiles → P6 contrast certification. The data-source contract is in data spec §0. Generation is **loopable/resumable and additive** — a persistent carry-over ledger lets later loops continue without redoing work and enrich cells (multiple songs per cell) — and catalog composition prioritizes **Japanese (JA-primary, EN fallback)** with `language`/`era` as first-class axes. The integrity firewall holds: **catalog selection is coverage-driven, never score-driven**, and the LLM judges expected **test-case labels blind** with the P6 score as a cross-check. P2 also emits a labeled **test-case set** alongside the label-free catalog. **Execution uses no LLM API** (no OpenAI/Anthropic keys): deterministic stages are **standalone Python CLIs**, the LLM stages are performed by the **interactive agent in the Claude Code terminal** via a file handoff (input → prompt → schema-checked output), and a Claude Code skill drives end-to-end; the CLIs also run in a bare terminal so the reproducibility-transform test needs no agent. The `real→12-vocab` genre map is **sub-first** and the `genre_affinity_v1` vocabulary stays frozen at 12 (design §13). Suggested build staging: **P2a** catalog (S0–S6), **P2b** worlds/profiles (S7), **P2c** LLM judge + certification + test cases (S8–S9).
...
- Implement the **offline generator tool** (a repository-defined command producing committed, versioned JSON) with the three staged passes from the data spec:
  - **PASS 1 — Track objects:** allocate stable synthetic Artist/Album/Track IDs (all prefixed `synthetic-`), keep real names and release data from the Soundcharts source, consistent artist references, exact `spotify_track` field names/types, `.invalid` URLs, `is_playable: true` / `is_local: false` for standard records. Free-form prose rejected.
...
- ~~Implement in-app catalog editing (`spotify_track` / `spotify_audio_features` / `simulation_flags`)~~ **DEFERRED (owner decision, 2026-07-16 — see §17 and design §"Scope Revision").** The song catalog is built and frozen in P2 and is **read-only** in P3, loaded with provenance; it is changed only by re-running P2. In its place P3 (a) makes **driver profiles a first-class, saveable, reusable store** and (b) **wires the real transparent content selector** so profile/situation changes produce a visibly different content proposal over the frozen catalog. The service selector stays mock.
- Implement world/history editing (`direct_item_history`, exact `oshi_id`, service lifecycle state) referencing (read-only) catalog IDs.
- Implement the opt-in `genre_affinity_v1` world fields (`usage_by_genre`, `scene_genre_usage`) as an extension toggle.
- Keep algorithm parameters/hyperparameters **distinct from world features** as setup surfaces. *(RECONCILED — P3, see `docs/superpowers/specs/2026-07-16-proposal-p3-editable-world-design.md` §2 D8 / §9.)* P1 (approved via `ui-mockup.html`) reversed the original 4-panel layout to **3 panels**: the **① World panel owns the world model only** (control inputs + situation + driver profile) and each selector panel (②/③) owns its own parameters/hyperparameters. The invariant here — params/hyperparameters are non-feature setup, kept separate from world features — is satisfied by that per-panel setup; **no separate 4th "Setup" panel is reintroduced** in P3.
...
- **Post-rest candidate count (4 vs 5).** *(RESOLVED — P1, see `docs/superpowers/specs/2026-07-16-proposal-p1-screen-foundation-design.md` §2 D3 / §9.)* The spec §7.5 matrix and Appendix A.0 originally listed four `after_rest_before_restart` services; the **service algorithm doc (§2.4/§5.2.4) deliberately adds a fifth**, `call_response_stopped`, citing Slides 38 and 40. **Resolution:** adopt **5** — the matrix is the resolved-allowed-set the P5 service selector consumes, the `ServiceId` enum (frozen in P0.5) already contains `call_response_stopped`, and it is eligibility-only under P1's mocks. The frozen v1 matrix artifact (`proposal_contracts/matrix/purpose_stage_matrix.v1.json`), spec §7.5, and Appendix A.0 all now list five.
...
- **Base-seed location and shape.** *(RESOLVED — P3, see `docs/superpowers/specs/2026-07-16-proposal-p3-editable-world-design.md` §9.)* The committed generator seeds (`generation_workspace/worlds.json` / `contrast_pairs.json`, in the generator's nested `trigger`/`driver`/`environment`/… shape) are **incomplete relative to the full Appendix A.1/A.2 feature contract** and live in a build-workspace directory the app should not read. **Resolution:** at authoring time they are **promoted** into a stable, app-readable `proposal_contracts/seeds/` directory in the canonical `SeedWorld` shape (control_inputs + situation + driver_profile), **completed** so every A.1/A.2 field is initialized (§5: "each seed initializes all fields"). The generation workspace stays a build area; a golden test guards the promotion.
- **P3 catalog editing deferred; content selector wired forward (owner scope decision, 2026-07-16).** *(RESOLVED — P3, see `docs/superpowers/specs/2026-07-16-proposal-p3-editable-world-design.md` "Scope Revision" and `specs/014-proposal-p3-editable-world/spec.md` Clarifications.)* §5 originally listed **in-app catalog editing** ("saving produces a new dataset version + hash") as a P3 deliverable. **Resolution:** the owner decided the song catalog is built and frozen in **P2** and is **read-only** in P3 (changed only by re-running P2). Catalog editing and its derived-version/hash acceptance criterion are **deferred** out of P3. To keep P3 a *meaningful, demonstrable* slice rather than invisible plumbing, P3 instead (a) elevates **driver profiles to a first-class save/list/reuse store** and (b) **pulls the real transparent CONTENT-selector wiring forward** from the later P6-wiring milestone, so a different driver profile yields a visibly different content proposal over the frozen catalog. The **service** selector stays mock (real service selector remains P5); **no** eligibility narrowing or journey engine is added (those remain P4). The determinism/grounding gates still hold: the frozen catalog is the replay boundary and no live model/network call occurs.
- **World model vs algorithm parameters (Panel ② interpretation).** *(RESOLVED — P3, design §2 D1/D8 / §9.)* The world model (control inputs + situation + driver profile — where **preference and history belong to the driver profile**) is owned by the **① World panel**; algorithm parameters/hyperparameters remain in the ②/③ selector panels. "Additional proposed" is a per-field **provenance origin** (`proposed_addition`), not a panel or an algorithm concept; `genre_affinity_v1` is an **opt-in driver-profile extension** scored only by the content algorithm. See the §5 Panel-② reconciliation above.
- **Missing spec cross-reference.** *(RESOLVED — P4, see `docs/superpowers/specs/2026-07-16-proposal-p4-eligibility-journey-design.md` §3/§11.)* The consolidated spec now cross-references the service algorithm doc at §11 (line "The complete normative design is `…aica_transparent_service_proposal_algorithm.md`"); its §7 (Hard eligibility) is the normative source P4's eligibility resolver consumes.
- **Machine-readable service capabilities.** *(RESOLVED — P4, design §4/§5 D5.)* Spec §7.1/§7.2 described screen-dependence, stopped-only, and lighting compatibility only in prose. P4 freezes them as a versioned artifact `proposal_contracts/service_capabilities/service_capabilities.v1.json` (per-`ServiceId`), the source of the motion/capability narrowing step in §6.1. A golden artifact test keeps it aligned with §7.1/§7.2.
- **Confidence shrinkage (service selector).** *(RESOLVED — P5, see `docs/superpowers/specs/2026-07-16-proposal-p5-transparent-service-selector-design.md` §7.)* P5's scope lists "confidence shrinkage" and an acceptance criterion "low-confidence rates have less effect than identical high-confidence rates," but the **authoritative service algorithm doc** states the *baseline* has no confidence feature (§5.4), lists `service_proposal_acceptance_confidence` / `service_recovery_confidence` as additional-simulator features the baseline disables (§5.6), and defers confidence-weighted sparse history (§19). **Resolution (owner decision):** implement it **now** as the opt-in, **off-by-default** `confidence_shrinkage_v1` extension (mirroring the content selector's `genre_affinity_v1`). With it **off** the two confidence fields stay `available_but_not_used` and the package reproduces the frozen doc byte-for-byte (the §10 worked example `+0.772349` and the §6.4 dominance table are unchanged); with it **on**, acceptance/recovery evidence is shrunk toward neutral by its confidence, satisfying the milestone criterion. A firewall test proves off == no-extension exactly. This amends algorithm §5.4/§5.6/§19; the baseline default is unchanged and the full probabilistic uncertainty package stays deferred.
...
| `rest_recommended` | `after_rest_before_restart` | `live_viewing`, `stretch_video`, `full_karaoke`, `oshi_reexperience`, `call_response_stopped` *(post-rest 4→5 resolved in P1; §17)* |
