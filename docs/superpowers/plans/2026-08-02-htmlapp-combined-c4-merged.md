...
- [ ] **Step 1** — enumerate; report before coding.
- [ ] **Step 2** — capture; failing tests; port.
- [ ] **Step 3** — **the LLM provider gap.** C3 Task 4 dropped Ollama (`provider: 'backend'`) because the offline build has no server, and defined what an htmlapp caller receives if it asks for it. Read that decision in C3's Task 4 report and mirror it here exactly. A merged endpoint that silently falls back to a template while reporting `backend` would misattribute which provider produced a rationale — the same defect C3 refused.
