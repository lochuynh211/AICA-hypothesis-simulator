...
- [x] **Step 4** — **the full-session op transcript.** The ADR requires 6 whole-session transcripts as C4's parity evidence, above the per-module goldens. Drive one complete Combined session — plan → create → tick to a fire → proposal-action → accept-rest → tick through recovery → explain → review-feedback — through the worker seam in TS and through the Python endpoints, and diff the op-by-op results. Per-module goldens cannot catch a state divergence that only appears when the ops run in sequence.
- [x] **Step 5** — **prove the non-persisting path stays non-persisting** end to end: run a quickview through the real seam, then list runs, and assert nothing was added.
- [x] **Step 6** — confirm no `backend`/Ollama code path shipped: grep the built output for any HTTP call to a local model server.
