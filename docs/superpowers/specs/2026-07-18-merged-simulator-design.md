...
2. **Slice 2 — full rest journey.** CORE ✅ **DONE** (commits `2259946..00f4a14`): the before→during→after rest chain in one proposal run (orchestrator auto-drives `rest_spot_arrived`/`rest_completed(post_rest)`/`recompute`, pausing at the after-rest proposal); **enriched recovery (§8)** — additive duration-scaled sleep (nap-minutes → nap-stage ticks) + time-scaled en-route rate with aggregate cap, flat-only path byte-identical; accept-rest UI (recovery-option + sleep-minutes); merged-log rest narration + recovery phases; after-rest dock-by-evidence-recency fix. Backend 2332 / frontend 644 / build clean; whole-branch opus review passed (verified live) after 1 fix. **Slice 2b (painter + hardening) ✅ DONE** (commits `07bea56..8c4ba4e`): route-conditions painter (§5) — `merged_painter` mountain-split + jam-conversion, `POST /api/merged-runs/plan` applies it, `RouteConditionsPainter` 2-dot sliders; MOVING-recovery `grants_moving_recovery` explicit flag (§8 landmine closed). Backend 2350 / frontend 647 / build clean. **Slice 2c ✅ DONE** (commits `ef9cdc5..68c1e63`): **quickview headless projection (§7.1)** — `iter_preview_ticks` generator (byte-identical `evaluate_preview`) + non-persisting `cache` kwarg + `services/merged_quickview.py` + `POST /api/merged-runs/quickview` (ephemeral, writes nothing) + a clickable `ScoreTimeline` projection strip (click a fire → inspect its proposal without Play); **correlation replay** — `GET /api/merged-runs/{id}` pure disk-read assembly + `createMergedReplaySource` (joins by `correlation_log[].trigger_tick_index`) + `MergedReplayViewer`/`MergedRunsScreen` + Live/Runs toggle, read-only. Backend 2381 / frontend 666 / build clean. **Feature 020 is now fully complete** (only optional follow-up: multi-fire `proposal_suppressed` surfacing).
3. **Slice 3 — monotony.** ✅ **DONE** (commits `2c3f747..0bdb8aa`): package-agnostic **monotony proxy signal (§6)** (`TickState.monotony_accrued_min` + `dynamic.monotonyLevel` in the tick engine; fed into `situation.monotony_level`), a monotony-firing scenario `scenarios/uc02_monotony_v0_1.json` + merged e2e proving `MONOTONY_PROPOSAL → inattentive_driving_prevention_recovery` / `active_driving_content` with real driving-content ranking and `monotony_level=100` and no rest chain, plus a bilingual purpose label in the merged log. Orchestrator + dock UNCHANGED (Slice-1 mapping + purpose-blind dock already handle it). Backend 2357 / frontend 648 / build clean.
