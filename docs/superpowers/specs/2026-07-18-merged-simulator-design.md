...
1. **Slice 1 — skeleton + rest happy-path.** Combined screen (3-panel, appMode, both stores + coordinator), `merged_runs` orchestrator + correlation record, six popup-setup shells, Play → trigger ticks → REST fire auto-opens service→content proposal → merged log. One preset route, one scenario, `rest_recommended` only. (Enriched recovery/quickview stubbed to today's fixed-once + trigger-only preview.)
2. **Slice 2 — full rest journey.** Rest-spot click → before/during/after chain; **enriched recovery (§8)** + sleep-minutes control; **quickview headless quick_check projection (§7.1)**; **route-conditions painter (§5)**; merged replay from correlation record.
