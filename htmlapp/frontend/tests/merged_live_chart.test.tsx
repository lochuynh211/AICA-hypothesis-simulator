/**
 * merged_live_chart.test.tsx (htmlapp port, fixbug-0806) — mirrors
 * `app/frontend/tests/merged_live_chart.test.tsx`'s coverage of the LIVE
 * driver-signal chart stacked under the Combined screen's projection strip.
 *
 ...
 *
 * htmlapp has no HTTP client to `vi.mock` — `state/mergedCoordinator.tsx` calls
 * the in-process TS engine through `api/mergedClient.ts` (`call('merged.tick',
 * ...)` etc.), so mocking that SAME module (`../src/api/mergedClient`) plays
 * the identical role the reference's `vi.mock('.../api/mergedClient', ...)`
 * plays against its HTTP layer — see `tests/merged_center.test.tsx` and
 * `tests/merged_quickview_gating.test.tsx` for the same pattern.
 */
...
describe('mergedLiveTimeline', () => {
  ...
  it('labels a rest dot with the elapsed minute of the FIRST tick to reach the spot', () => {
    ...
    const data = mergedLiveTimeline({
      trace: [
        traceEntry({ tick_index: 0, route_fraction: 0.55, time_min: 20 }),
        traceEntry({ tick_index: 1, route_fraction: 0.55, time_min: 40 }),
      ],
      restSpots: [spot],
      completed: false,
    })
...
function quickviewFixture(): MergedInstantResult {
  return {
    ...
    // The projection always carries driver signals, so the projection carries
    // the same lower band the live chart does — the two charts' score bands
    // only line up when both are laid out the same way.
    signal_series: [
