// htmlapp/frontend/tests/proposal_store_isolation.test.ts
/**
 * TS port slice of `app/frontend/tests/proposalStore.isolation.test.ts`'s
 * LOAD_CASE_WORLD reducer tests (mirrors app/frontend commit 40d2da5, "fix
 * bug", bug 3 — store/wiring half): a newly selected test case must reset
 * `world.situation` to a clean baseline before the case's own pinned
 * `SET_SITUATION_FIELD` overlays are applied — unlike `LOAD_PROFILE`, which
 * replaces only `world.driver_profile` and leaves whatever situation the
 * previously-selected case left behind.
 *
 * Only the LOAD_CASE_WORLD-specific assertions are ported here (this repo's
 * existing runStore-isolation coverage for the OTHER proposalStore actions
 * lives in `tests/proposal_stores.test.ts` / elsewhere) — the reference
 * file's SET_LANGUAGE/SET_SITUATION_FIELD isolation checks and the
 * import-source check are not duplicated.
 */
...
describe('proposalStore LOAD_CASE_WORLD (bug 3, store/wiring half)', () => {
  it('replaces situation + driver_profile together, sets selectedProfileId, and syncs motion_state without mutating runStore', () => {
  ...
  it('falls back to the current motionState when the case situation omits motion_state', () => {
    const { result } = renderHook(
      () => ({ proposal: useProposalStore(), run: useRunStore() }),
      { wrapper },
    )
