...
_p("preset-monotone-highway-energize", family="mood_coherence",
   ...
   situation={"drowsiness_level": 55, "fatigue_level": 20, "monotony_level": 92,
...
_p("preset-late-night-winddown", family="mood_coherence",
   ...
   situation={"drowsiness_level": 30, "fatigue_level": 85, "monotony_level": 20,
...
# ---- Family B: directional hypothesis toggle (2a ⇄ 2b, same world) ----
# The world is driven by the DIRECTIONAL features (night/traffic/fatigue) that the
# hypothesis flips — and drowsiness/monotony are LOW so they don't force high-arousal
# under both hypotheses (they reward high arousal regardless), and so the top service
# stays modest enough that the mood flip decides the #1 song. Pure mood (no oshi/genre).
_HYP_SIT = {"drowsiness_level": 15, "fatigue_level": 40, "monotony_level": 15,
            "traffic_state": "congested", "night_state": "night", "road_type": "local",
            "route_tags": ["urban"], "destination_tags": ["home"]}
_HYP_PROF = {"oshi_registered": False, "oshi_mode": "off", "genre_affinity_v1_enabled": False}
_p("preset-drowsy-soothe", family="directional_hypothesis",
   contrast_with="preset-drowsy-keepalert",
   label={"en": "Tired night — soothe (default)", "ja": "夜間の疲労 — 癒し（既定）"},
   brief={"en": "A tired night drive in city congestion — the conditions where the 'soothe vs energize' hypothesis matters. Under the default 'soothe_destress', the calmer, mid-arousal side of the ranking wins. Its partner preset runs the identical world with the hypothesis flipped and a higher-energy track wins instead.",
          "ja": "夜間・市街地の渋滞での疲れた運転 — 『癒しか覚醒か』仮説が効く条件。既定の『soothe_destress』では落ち着いた中アロusal寄りが上位に。対のプリセットは同一状況で仮説を反転し、より高エネルギーな曲が上位に。"},
   situation=_HYP_SIT, profile=_HYP_PROF,
   control={"trigger_purpose": "inattentive_driving_prevention_recovery",
            "lifecycle_stage": "active_driving_content"},
   expectation=_exp("under soothe_destress the top track differs from keep_alert on the identical world",
                    top={"arousal_band": "mid"}, top_fit_min=0.08, gradient="arousal_down_implies_fit_up",
                    service=["humming_karaoke", "quiz", "call_response_driving", "music_playlist", "radio_style"]))

_p("preset-drowsy-keepalert", family="directional_hypothesis",
   contrast_with="preset-drowsy-soothe",
   label={"en": "Tired night — keep alert", "ja": "夜間の疲労 — 覚醒維持"},
   brief={"en": "Identical world to 'soothe', but the reviewer flips the directional hypothesis to 'keep_alert' (a per-preset override). Now a higher-arousal track is favored to fight the tiredness — the same inputs, a different, explainable #1.",
          "ja": "『癒し』と同一の状況だが、方向性仮説を『keep_alert』に切替（プリセット単位のオーバーライド）。眠気対策としてより高アロusalな曲が1位に — 同じ入力で結果が変わる。"},
   situation=_HYP_SIT, profile=_HYP_PROF,
   control={"trigger_purpose": "inattentive_driving_prevention_recovery",
            "lifecycle_stage": "active_driving_content"},
   overrides={"content": {"directional_hypothesis": "keep_alert"}, "service": None},
   expectation=_exp("flipping to keep_alert makes a higher-arousal track top the identical world",
                    top={"arousal_band": "high"}, top_fit_min=0.16, gradient="arousal_up_implies_fit_up",
                    service=["humming_karaoke", "quiz", "call_response_driving", "music_playlist", "radio_style"],
...
_OSHI_SIT = {"drowsiness_level": 35, "fatigue_level": 30, "monotony_level": 45,
             "traffic_state": "normal", "night_state": "day", "road_type": "highway"}
...
_GEN_SIT = {"drowsiness_level": 40, "fatigue_level": 30, "monotony_level": 55,
            "traffic_state": "normal", "night_state": "day", "road_type": "highway"}
...
_p("preset-coastal-cruise", family="route_genre",
   ...
   situation={"road_type": "local", "route_tags": ["coastal"], "destination_tags": ["coast"],
              "monotony_level": 40, "drowsiness_level": 25, "fatigue_level": 25, "night_state": "day"},
...
_p("preset-mountain-pass", family="route_genre",
   ...
   situation={"road_type": "mountain", "route_tags": ["mountain"], "destination_tags": ["nature"],
              "monotony_level": 35, "drowsiness_level": 25, "fatigue_level": 30, "night_state": "day"},
...
_p("preset-child-family-drive", family="passenger_genre",
   ...
   situation={"child_present": True, "multiple_passengers": True, "road_type": "local",
              "night_state": "day", "monotony_level": 30, "drowsiness_level": 20, "fatigue_level": 20},
...
_p("preset-reststop-full-karaoke", family="singability_service",
   ...
   situation={"drowsiness_level": 20, "fatigue_level": 20, "monotony_level": 20, "night_state": "day",
...
_p("preset-high-recovery-regular", family="history_mechanics",
   ...
   situation={"drowsiness_level": 45, "fatigue_level": 45, "monotony_level": 45, "night_state": "day",
              "road_type": "highway"},
...
_p("preset-recently-played-fatigue", family="history_mechanics",
   ...
   situation={"drowsiness_level": 45, "fatigue_level": 45, "monotony_level": 55, "night_state": "day",
              "road_type": "highway"},
...
_p("preset-coldstart-neutral", family="baseline",
   ...
   situation={"drowsiness_level": 30, "fatigue_level": 25, "monotony_level": 40, "night_state": "day",
              "road_type": "highway"},
...
_p("preset-anime-fan-event-night", family="combo",
   ...
   situation={"night_state": "night", "destination_tags": ["event", "oshi_venue"],
              "route_tags": ["highway"], "monotony_level": 55, "drowsiness_level": 35, "fatigue_level": 35},
