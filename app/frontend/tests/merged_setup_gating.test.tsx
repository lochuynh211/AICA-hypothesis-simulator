/**
 * merged_setup_gating.test.tsx (fixbug-0806, Task 3) — the Combined screen's
 ...
 *
 * Kept in a SEPARATE file from `merged_quickview_gating.test.tsx` rather than
 * appended to it: that file already declares a hard-replacement
 * `vi.mock('../src/api/mergedClient', () => ({...}))` (no `buildMergedPlan`,
 * no `acceptRest`/`declineRest`) for its coordinator/BusyOverlay tests, and
 * `vi.mock` calls are hoisted per-module-per-file — a second, differently
 * shaped mock for the same path in the same file isn't possible, and
 * reshaping the existing one risked the already-passing Task 1/2 tests. This
 * file mirrors `merged_setup.test.tsx`'s mock scaffold instead (spread the
 * real module via `orig`, override only the network boundary) so
 * `MergedSetupPanel` mounts with its real Modal/store wiring.
 */
 ...
 * mirrors `merged_setup.test.tsx`'s `fillSetup`. */
