...
/** Overrides the default (no-presets) `getScenario` mock with a scenario that
 *  carries a TIME-based `presets.traffic_events` entry — the shape
 *  `scenarioJamRangesKm` (MergedSetupPanel) inverts to a km range for display. */
function mockScenarioWithJamPreset(durationMin: number) {
  vi.mocked(getScenario).mockResolvedValue({
    id: 'scn_fatigue_1',
    is_night: false,
    familiar_route: false,
    ...
    presets: {
      traffic_events: [
        { id: 'j', start_min: 0, duration_min: durationMin, affected_segment_id: 'x', speed_kph: 8 },
      ],
      weather_events: [],
    },
  } as unknown as Awaited<ReturnType<typeof getScenario>>)
...
describe('MergedSetupPanel — route-conditions painter', () => {
  ...
  it('bridges a scenario-preset jam (time-based) to the map without a painted range, and keeps the plain plan-build path on Play (bug 1)', async () => {
    mockScenarioWithJamPreset(45) // 45/90 * 120km = 60km
    renderPanel()
    await fillSetup()

    // No painting at all — the scenario's own traffic_events preset is the
    // only source of a jam here.
    await waitFor(() => expect(runStateRef.current?.mergedJamRangesKm).toEqual([[0, 60]]))
  ...
  it('shows the scenario-derived jam as the painter readout fallback when nothing is painted, but never for mountain (bug 3)', async () => {
    mockScenarioWithJamPreset(90) // full route: 90/90 * 120km = [0, 120]
    renderPanel()
    await fillSetup()
    ...
    expect(await screen.findByTestId('jam-range-readout')).toHaveTextContent(/0.*120/)
    expect(screen.getByTestId('mountain-range-readout')).toHaveTextContent(/0.*0/)
  })

  it('a painted jam REPLACES the scenario-preset jam on the map bridge (does not union)', async () => {
    mockScenarioWithJamPreset(90) // full route: 90/90 * 120km = [0, 120]
    renderPanel()
    await fillSetup()
    ...
    fireEvent.click(screen.getByTestId('edit-situation'))
    fireEvent.click(await screen.findByTestId('setup-detailed-toggle'))
    await screen.findByTestId('jam-range-start')

    fireEvent.change(screen.getByTestId('jam-range-start'), { target: { value: '10' } })
    fireEvent.change(screen.getByTestId('jam-range-end'), { target: { value: '20' } })

    // The painted range must REPLACE the scenario-wide [0, 120] jam, not union
    // with it — otherwise the scenario's stale full-route jam stays drawn on
    // the map underneath the painted one and the painted start/end never
    // appears to update.
    await waitFor(() => expect(runStateRef.current?.mergedJamRangesKm).toEqual([[10, 20]]))
  })

  it('reaches 1 km granularity on the jam slider (bug 2)', async () => {
    // Default (no-presets) scenario mock from setupMocks() — STEP_KM=1 is a
    // property of the painter itself, independent of scenario presets.
    renderPanel()
    await fillSetup()
    ...
    // 3 and 7 are not multiples of 5 — under the old STEP_KM=5 these would
    // have snapped to 5 and either 5 or 10 (min-gap dependent), never landing
    // on exactly 3..7.
    fireEvent.change(screen.getByTestId('jam-range-start'), { target: { value: '3' } })
    fireEvent.change(screen.getByTestId('jam-range-end'), { target: { value: '7' } })
