...
# ---------------------------------------------------------------------------
#
# The Japanese half of every entry below MUST match
# ``app/frontend/src/lib/review/reviewVocabulary.ts`` (``FIELD_NAMES`` /
# short-leaf entries) — that table is the single reference wording for the
# Combined screen's feature vocabulary, and this dict is its backend mirror
# for the LLM-explanation pipeline (deterministic ``template()`` fallbacks
# render these labels directly to the screen, so a mismatch here is a visible
# vocabulary drift, not just an internal inconsistency).
FEATURE_LABELS: dict[str, dict[str, str]] = {
    ...
    "child_present": {"ja": "子供の同乗", "en": "child present"},
    "multiple_passengers": {"ja": "複数人の同乗", "en": "multiple passengers"},
    "oshi_registered": {"ja": "推しの登録", "en": "oshi registered"},
    "oshi_mode": {"ja": "推し優先モード", "en": "oshi mode"},
    "service_recency_state": {"ja": "サービスの未利用期間", "en": "service recency"},
    "service_usage_level": {"ja": "サービスの利用頻度", "en": "overall service usage"},
    ...
    "content_tag_usage_level": {"ja": "ジャンルの利用頻度", "en": "genre/tag play frequency"},
    "content_tag_recency_state": {"ja": "ジャンルの未再生期間", "en": "genre/tag recency"},
    "scene_content_tag_usage_level": {"ja": "場面別のジャンル利用", "en": "scene-specific genre usage"},
    "usage_by_genre": {"ja": "ジャンル別の利用", "en": "usage by genre"},
    "scene_genre_usage": {"ja": "場面別のジャンル利用", "en": "scene genre usage"},
    ...
    "changed_from_items": {"ja": "切替履歴", "en": "recently switched away from"},
