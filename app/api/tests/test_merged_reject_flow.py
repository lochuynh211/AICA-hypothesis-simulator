...
def rest_plan_id() -> str:
    resp = client.post(
        ...
        json={
            ...
            # These are rest-JOURNEY mechanics tests (reject-at-step, monotony
            # acknowledge, content relief) — orthogonal to the rest-after-monotony
            # spacing rule, which owns its own coverage in test_rest_min_gap_guard.py
            # / test_forecast_parity.py. NRI's manifest now defaults
            # `rest_min_gap_after_monotony_min` to 20, and on this scenario that
            # delays the first REST fire ~12 min past the point where the sole rest
            # spot is still ahead of the vehicle — so the rest never surfaces
            # actionably where these tests expect it. Disabling the gap (0 = off)
            # restores the exact pre-feature timing these tests were authored
            # against, mirroring test_forecast_parity's own override.
