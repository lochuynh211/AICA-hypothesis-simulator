...
- [ ] T006 Implement `app/api/aica_api/services/dataset_catalog_registry.py` (read-only loader over `settings.proposal_dataset_dir`, `get_catalog`/`list_datasets`/provenance) to pass T005; import `mdg.validator.validate_song` for read-side validation only.
...
- [ ] T018 [US1] Implement `app/api/aica_api/services/world_validation.py` (`validate_world(world, catalog)`) reusing `mdg.worlds.validate_world_references`, to pass T017.
